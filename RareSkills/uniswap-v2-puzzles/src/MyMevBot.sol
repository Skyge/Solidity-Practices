...
contract MyMevBot {
    ...
    function performArbitrage() public {
        ...
        IUniswapV3Pool(flashLenderPool).flash(address(this), borrowAmount, 0, abi.encode(0));
    }
    ...
    function uniswapV3FlashCallback(uint256 _fee0, uint256, bytes calldata data) external {
        ...
        IUniswapV2Router(router).swapExactTokensForTokens(1000 * 1e6, 0, path, address(this), block.timestamp + 60);

        // Pay loan with fee to the flashLenderPool
