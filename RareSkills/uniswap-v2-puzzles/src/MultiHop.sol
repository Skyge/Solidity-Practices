...
contract MultiHop {
    ...
    function performMultiHopWithRouter(address mkr, address weth, address elon, uint256 deadline) public {
        ...
        // Swap path: MKR -> WETH -> ELON
        // Approve the router to spend MKR
        IERC20(mkr).approve(router, IERC20(mkr).balanceOf(address(this)));
        // Swap MKR for ELON
        address[] memory path = new address[](3);
        path[0] = mkr;
        path[1] = weth;
        path[2] = elon;
        IUniswapV2Router(router).swapExactTokensForTokens(IERC20(mkr).balanceOf(address(this)), 0, path, address(this), deadline);
    }
}
